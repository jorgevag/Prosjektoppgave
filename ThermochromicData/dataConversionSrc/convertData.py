...
#               -o <outputFile>,                                                                                         #
#                                          Here y1(x1), y2(x2), etc...                                                   #
# Examples:                                                                                                              #
#           ./convertData.py -i refile.txt,imfile.dat,s=2,d=.,0,1   -o outputfile.dat                                    #
#           ./convertData.py -i refile.txt,s=2,d=.,0,1  -i imfile.dat,s=2,d=comma,0,4 -o outputFile.nk                   #
#                                                                                                                        #
# Please Notice (regarding the delimiter input):                                                                         #
#           If delimiter is whitespace(space or tab), then don't specify it (leave it out).                              #
#           Due to the ',' separates the -i input, you can't use it as a delimiter input. So                             #
#           if you want to use ',' as delimiter, then write 'd=comma'.                                                   #
#                                                                                                                        #
# Help:                                                                                                                  #
#           ./convertData.y -h                                                                                           #
#                                                                                                                        #
...
if len(sys.argv) < 2:
    ...
    print '                                                                                                                       '
    print 'Please Notice (regarding the delimiter input):                                                                         '
    print "          If delimiter is whitespace(space or tab), then don't specify it (leave it out).                              "
    print "          Due to the ',' separates the -i input, you can't use it as a delimiter input. So                             "
    print "          if you want to use ',' as delimiter, then write 'd=comma'.                                                   "
    print '                                                                                                                       '
    print 'Format:                                                                                                                '
    print "          -i  <realDataFile>,<imaginaryDataFile>,(s=int),(d='char'),unit=string,isPermittivity=bool,X_Re,Y_Re,X_Im,Y_Im"
    print '              -o <outputFile>,                                                                                         '
    print '                                         Here y1(x1), y2(x2), etc...                                                   '
    print 'Examples:                                                                                                              '
    print '          ./convertData.py -i refile.txt,imfile.dat,s=2,d=.,0,1   -o outputfile.dat                                    '
    print '          ./convertData.py -i refile.txt,s=2,d=.,0,1  -i imfile.dat,s=2,d=comma,0,4 -o outputFile.nk                   '
    print '                                                                                                                       '
    sys.exit()
...
for flag, arg in flags:
    ...
    # If Output Flag -> get the output filename #
    elif flag in ('-o','--ofile'):
        ofile = arg
...
if( isPermittivity ): # data is permittivity
    ...
    #epsilon0 = scipy.constants.epsilon_0 # I don't think I need this after all
    #n = numpy.sqrt( (absEpsilon + re)/2.0*epsilon0 ) #I think maybe the plots are given in relative permittivity!
    #k = numpy.sqrt( (absEpsilon - re)/2.0*epsilon0 )
    n = numpy.sqrt( (absEpsilon + re)/2.0)
    k = numpy.sqrt( (absEpsilon - re)/2.0)
