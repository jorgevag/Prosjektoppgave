...
if len(sys.argv) < 1:
    ...
    print 'Run:                                        '
    print '     ./plotBareSubstrate.py -i ifile1.txt,ifile2.dat'
    sys.exit()
...
if(parallel):
    if(plotR):
        ...
        for ifile in ifiles:
            ...
            pyplot.plot(data[:,0], data[:,3]**2 + data[:,4]**2, label='$R_p$ '+str(ifileNameTemperature))
    ...
    if(plotT):
        ...
        for ifile in ifiles:
            ifileNameTemperature = ifile.split('/')[-1].split('.')[0].split('_')[-1] #get data temperature
            data = numpy.loadtxt(ifile, skiprows=skipRows, delimiter = dataDelimiter) # Load file data
            pyplot.plot(data[:,0], data[:,11]**2 + data[:,12]**2, label='$T_p$ '+str(ifileNameTemperature))
            pyplot.legend(loc='best')
    ...
    if(plotA):
        ...
        for ifile in ifiles:
            ifileNameTemperature = ifile.split('/')[-1].split('.')[0].split('_')[-1] #get data temperature
            data = numpy.loadtxt(ifile, skiprows=skipRows, delimiter = dataDelimiter) # Load file data
            pyplot.plot(data[:,0], 1-(data[:,3]**2 + data[:,4]**2)-(data[:,11]**2 + data[:,12]**2), label='1-$T_p$-$R_p$ '+str(ifileNameTemperature))
            pyplot.legend(loc='best')
...
if(senkrecht):
    if(plotR):
        ...
        for ifile in ifiles:
            ifileNameTemperature = ifile.split('/')[-1].split('.')[0].split('_')[-1] #get data temperature
            data = numpy.loadtxt(ifile, skiprows=skipRows, delimiter = dataDelimiter) # Load file data
            pyplot.plot(data[:,0], (data[:,7]**2 + data[:,8]**2), label='$R_s$ '+str(ifileNameTemperature))
            pyplot.legend(loc='best')
    ...
    if(plotT):
        ...
        for ifile in ifiles:
            ifileNameTemperature = ifile.split('/')[-1].split('.')[0].split('_')[-1] #get data temperature
            data = numpy.loadtxt(ifile, skiprows=skipRows, delimiter = dataDelimiter) # Load file data
            pyplot.plot(data[:,0], (data[:,15]**2 + data[:,16]**2), label='$T_s$ '+str(ifileNameTemperature))
            pyplot.legend(loc='best')
    ...
    if(plotA):
        ...
        for ifile in ifiles:
            ifileNameTemperature = ifile.split('/')[-1].split('.')[0].split('_')[-1] #get data temperature
            data = numpy.loadtxt(ifile, skiprows=skipRows, delimiter = dataDelimiter) # Load file data
            pyplot.plot(data[:,0], 1-(data[:,7]**2 + data[:,8]**2)-(data[:,15]**2 + data[:,16]**2), label='1-$T_s$-$R_s$ '+str(ifileNameTemperature))
            pyplot.legend(loc='best')
