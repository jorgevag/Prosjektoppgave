...
contains
  ...
  !---------------------------------------!
  Subroutine SFL_Error_Init_Local__(error)        ! IS this routine needed...?
  !---------------------------------------!
    !
  ...
  End Subroutine SFL_Error_Init_Local__
  !------------------------------------!
  ...
  !----------------------------------------------!
    ...
    If ( (.not.error%Local_init) .and. (error%Local_Copy) )   &
         call SFL_Error_Init_Local__(error)
    ! Local definitions
    ...
    if (error%print_level <= 0)& 
         Write(error%unit,*) " +++ Procedure ",Trim(Adjustl(uppercase__(error%generic_name))), &
         " entered!"
  ...
  !--------------------------------!
    ...
    if (error%level >= error%print_level) then
       ...
       write(str,*) Trim(AdjustL(uppercase__(error%generic_name)))
       Write(error%unit,*)
  ...
  !------------------------------------------------!   
    ...
    ! Local
    Integer :: i
    Write(*,*) "INTERNAL ERROR : "// trim(adjustl(uppercase__(routine)))
  ...
  !------------------------------------------------!
  function uppercase__(in_string) result(out_string)
  !------------------------------------------------!
     !---------------------------------------------------------------------
     ! ROUTINE   :  Upper2lower 
     ! TYPE      :  Function
