...
Module SFL_Util
  ...
  !
  ! It also used some a non-intrinsic F90 version of the
  ...
  Public :: SFL_YearString         ! Create a YearString 
  ...
  Public :: SFL_BaseName           ! Extracts the base name from an input path

  Public :: SFL_DirName            ! Extracts the path from an input string

  Public :: SFL_Join_Path          ! Joint the path and the filename into a valid path 

  Public :: SFL_DirSep             ! Function returning the OS depende directory separator
  ...
  ! --- Some local defaults
  Character(len=*), parameter :: OS_SEPARATOR_DEFAULT="/"
  Character(len=*), parameter :: OS_DEFAULT="unix"
...
contains
  ...
  !----------------------------------------!
  Function SFL_DirSep(OS)  Result(Separator)
  !----------------------------------------!
    !
    ! --- This function returns the Directory Separator
    !     The Optional argument OS = ( unix, linux, win, windows,dos)
    !     specifies the operating system to be used.
    !     by default OS is unix
    !
    !     We have for :   Unix   /
    !                     win    \
    !
    !     Ingve Simonsen, UCI, Jul. 2008
    !
    Character(len=*), Intent(in), optional :: OS
    Character(len=1)                       :: Separator

    if (present( OS) ) then
  
       select case( LowerCase_Scalar(OS) ) 
          case("win","windows""dos") 
             Separator ="\\"
          case ("unix","linux")
             Separator = "/"
          case default 
             Write(*,*) " WARNING : OS not recognized using DEFAULT (unix)!"
             Separator = OS_SEPARATOR_DEFAULT    
          end select
    else
       ! The default value....
       Separator = OS_SEPARATOR_DEFAULT    
  ...
  End Function SFL_DirSep
  !----------------------!


  !--------------------------------------------------------!
  Function SFL_BaseName(Full_Path, Suffix)  Result(FileName)
  !--------------------------------------------------------!
    !
    ...
    !    
    !     This function MIMICS the Python/PHP function of the same name....
    !     NOTE this is different from the unix command of the same name...
    !
    ...
    DirSep   = SFL_DirSep( OS_DEFAULT )
    FileName = trim(adjustl(Full_Path)) 
    length   = len(FileName)

    ! --- Remove the path
    ipos = INDEX(FileName, DirSep , BACK=.true.)
    ! --- if last character is "/" return the empty string....
    if (ipos == length) then
       FileName = ""
       return
    endif   
    if ( (ipos /= 0) .and. (ipos<length) )  &
  ...
  !-------------------------------------------!
  Function SFL_DirName(Full_Path)  Result(Path)
  !-------------------------------------------!
    !
    ! --- This routine extracts the UNIX path from the input character string.
    !     If no path is given in the input, or the input string is empty, 
    !     the default value of "." (current directory) is returned....
    !    
    !     This function MIMICS the python/PHP function of the same name....
    !
    !     Ingve Simonsen, UCI, Irvine, Jul. 2008.
    !
    !     SEE ALSO : SFL_BaseName
    !
    implicit none
    character(len=*),                     Intent(In)  :: Full_Path
    character(len=max(len(Full_Path),2))              :: Path
    ! Local 
    Character(len=1)               :: DirSep
    character(len=len(Full_Path))  :: FileName        
    Integer                        :: ipos, length
    ...
    ! --- Remove the filename (including the /)
    ipos = INDEX(FileName, DirSep , BACK=.true.)
    if ( (ipos /= 0) .and. (ipos<=length) )  then
       Path = FileName(1:ipos-1)
    else
       Path = "."
    endif
  ...
  !-----------------------------------------------------------!
  Function SFL_Join_Path(Path, Filename)  Result(Path_FileName)
  !-----------------------------------------------------------!
    !
    ! --- This routine joins the path and the filename into a valid
    !     path for the OS in question
    !
    !     Ingve Simonsen, UCI, Irvine, Jul. 2008.
    !
    character(len=*), Intent(In)             :: Path
    character(len=*), Intent(In)             :: Filename
    Character(len=len(Path)+len(Filename)+2) :: Path_FileName
    ! Local
    Integer :: iloc
    Character(len=len(path)+2) :: Path_local
    
    
    ! --- If the traling character of path is not "/", add one
    !     This is to make sure that the call to SFL_DirName
    !     will not remove the last part of the directory....
    iloc = len(path)
    if ( Path(iloc:iloc) /= SFL_DirSep(OS_DEFAULT)) then
       Path_Local = trim(adjustl(Path)) // SFL_DirSep(OS_DEFAULT)
    else
       Path_Local = trim(adjustl(Path)) 
    endif
    
    ! --- Join the paths
    Path_FileName = trim(adjustl( SFL_DirName(Path_Local))) // &
                      SFL_DirSep(OS_DEFAULT)                   // &
                      trim(adjustl(SFL_BaseName(Filename)))  
  
  End Function SFL_Join_Path
  !--------------------------!
  ...
  Subroutine SFL_YearString(Year,Time_String) 
    !
    ! Returns the year as defined by Time_String in the 
    ! format '11 Nov 2006  23:43:03.737' 
    ! (as returned by e.g.g SFL_TimeString)
    !
    ! If Time_String is not present, current time/year 
    ! is used.... 
    !
    !
    ! I. Simonsen, Irvine, Jul 2009
    !
    Implicit None
    Character(len=4)            :: Year
    Character(len=*), optional  :: Time_String
    ! --- Local
    Character(len=30)        :: Time_String_Local
   
    if ( .not. present(Time_String) ) then
       ! --- Get current year
       call SFL_TimeString ( Time_String_Local )
    else
       ! --- Using Input String
       Time_String_Local = Time_String
    End if

    ! --- Adjust the string to the left....
    !     BUG : this cuse the 8:11 interval to be incorrect for single didit dates....
    !Time_String_Local  = adjustl(Time_String_Local)

    ! --- Extract the year....
    Year = Time_String_Local(8:11)

  End Subroutine SFL_YearString
  ...
  Subroutine SFL_TimeStamp(unit)
    ...
    Use SFL_ISO_FORTRAN_ENV, only : Output_Unit
    Implicit None
...
!!$
!!$
!!$    Write(*,*) SFL_BaseName("/usr/local/etc/mydir.txt")
!!$    Write(*,*) SFL_BaseName("/usr/local/etc/mydir")
!!$    Write(*,*) SFL_BaseName("./mydir.txt")
!!$    Write(*,*) SFL_BaseName("mydir.txt")
!!$    Write(*,*) SFL_BaseName("/usr/local/etc/")
!!$    Write(*,*) SFL_BaseName("/usr/local/etc/ ")
!!$    Write(*,*)
!!$    Write(*,*) SFL_DirName("/usr/local/etc/mydir.txt")
!!$    Write(*,*) SFL_DirName("/usr/local/etc/kast/")
!!$    Write(*,*) SFL_DirName("/usr/local/etc/kast")
!!$    Write(*,*) SFL_DirName("kast.dat")
!!$    Write(*,*) SFL_DirName(" ")
!!$    Write(*,*) SFL_DirName("")
!!$
!!$    Write(*,*) SFL_DirSep()
!!$    Write(*,*) SFL_DirSep("Unix")
!!$    Write(*,*) SFL_DirSep("win")
!!$    Write(*,*) SFL_DirSep("Unixsdsds")
!!$
!!$    Write(*,*) SFL_Join_Path("/etc/", "test.dat")
!!$    Write(*,*) SFL_Join_Path("/etc/", "/test.dat")
!!$    Write(*,*) SFL_Join_Path("/etc", "test.dat")
!!$    Write(*,*) SFL_Join_Path("/etc", "/test.dat")
!!$
