...
!---------------------!
  ...
  Real(wp)   :: EpsAbs = 1.e-6_wp                           ! Absolute precison in integral
  Real(wp)   :: EpsRel = 1.e-4_wp                           ! Relative precision in integral
...
Contains 
  ...
  Subroutine Quadpack(F,x1,x2,Int)
    ...
    select case(wp)
    case(sp)
       !NICK   Call qag(F,x1,x2,EpsAbs$,EpsRel$,key,Int,abserr,neval,ier,limit,lenw,last,iwork,work)
  ...
  subroutine qag ( f, a, b, epsabs, epsrel, key, result, abserr, neval, ier )
    ...
    !  Parameters:
    !
    !    Input, external Real(wp) F, the name of the function routine, of the form
    !      function f ( x )
    !      Real(wp) f
    !      Real(wp) x
    !    which evaluates the integrand function.
    !
    !    Input, Real(wp) A, B, the limits of integration.
    !
    !    Input, Real(wp) EPSABS, EPSREL, the absolute and relative accuracy requested.
    !
    !    Input, Integer(i4b) KEY, chooses the order of the local integration rule:
    ...
    !
    !    Output, Real(wp) RESULT, the estimated value of the integral.
    !
    !    Output, Real(wp) ABSERR, an estimate of || I - RESULT ||.
    !
    !    Output, Integer(i4b) NEVAL, the number of times the integral was evaluated.
    !
    !    Output, Integer(i4b) IER, return code.
    ...
    Integer(i4b), parameter :: limit = 500
  ...
  subroutine qage ( f, a, b, epsabs, epsrel, key, limit, result, abserr, neval, &
    ...
    !    6, 30 Gauss points, 61 Gauss-Kronrod points.
    !
    !    Input, Integer(i4b) LIMIT, the maximum number of subintervals that
    !    can be used.
    !
    !    Output, Real(wp) RESULT, the estimated value of the integral.
    !
    ...
    !    6, the input is invalid, because EPSABS < 0 and EPSREL < 0.
    !
    !    Workspace, Real(wp) ALIST(LIMIT), BLIST(LIMIT), contains in entries 1 
    !    through LAST the left and right ends of the partition subintervals.
    !
    !    Workspace, Real(wp) RLIST(LIMIT), contains in entries 1 through LAST
    !    the integral approximations on the subintervals.
    !
    !    Workspace, Real(wp) ELIST(LIMIT), contains in entries 1 through LAST
    !    the absolute error estimates on the subintervals.
    !
    !    Output, Integer(i4b) IORD(LIMIT), the first K elements of which are pointers 
    ...
    !
    !    Output, Integer(i4b) LAST, the number of subintervals actually produced 
  ...
  subroutine qagi ( f, bound, inf, epsabs, epsrel, result, abserr, neval, ier )
    ...
    !    which evaluates the integrand function.
    !
    !    Input, Real(wp) BOUND, the value of the finite endpoint of the integration
    !    range, if any, that is, if INF is 1 or -1.
    !
    !    Input, Integer(i4b) INF, indicates the type of integration range.
    !    1:  (  BOUND,    +Infinity),
    !    -1: ( -Infinity,  BOUND),
    !    2:  ( -Infinity, +Infinity).
    !
    ...
    !    Output, Integer(i4b) NEVAL, the number of times the integral was evaluated.
    !
    !    Output, Integer(i4b) IER, error indicator.
...
!!!  Real(wp) one_wp 
    ...
    call qk15i ( f, boun, inf, zero_wp, one_wp , result, abserr, defabs, resabs )
    !
    !  Test on accuracy.
    !
    last = 1
...
130 continue
  ...
  subroutine qagp ( f, a, b, npts2, points, epsabs, epsrel, result, abserr, &
    ...
    !    Input, Real(wp) A, B, the limits of integration.
    !
    !    Input, Integer(i4b) NPTS2, the number of user-supplied break points within 
    !    the integration range, plus 2.  NPTS2 must be at least 2.
    !
    !    Input/output, Real(wp) POINTS(NPTS2), contains the user provided interior
    ...
    !    Output, Integer(i4b) NEVAL, the number of times the integral was evaluated.
    !
    !    Output, Integer(i4b) IER, return flag.
...
210 continue
  ...
  subroutine qags ( f, a, b, epsabs, epsrel, result, abserr, neval, ier )
    ...
    !    Output, Integer(i4b) NEVAL, the number of times the integral was evaluated.
    !
    !    Output, Integer(i4b) IER, error flag.
    !                     ier = 0 normal and reliable termination of the
    !                             routine. it is assumed that the requested
    !                             accuracy has been achieved.
    !                     ier > 0 abnormal termination of the routine
...
140 continue
  ...
  subroutine qawc ( f, a, b, c, epsabs, epsrel, result, abserr, neval, ier )
    ...
    !    Input, Real(wp) A, B, the limits of integration.
    !
    !    Input, Real(wp) C, a parameter in the weight function, which must
  ...
  subroutine qawce ( f, a, b, c, epsabs, epsrel, limit, result, abserr, neval, &
    ...
    !    Input, Real(wp) A, B, the limits of integration.
    !
    !    Input, Real(wp) C, a parameter in the weight function, which cannot be
    !    equal to A or B.
    !
    !    Input, Real(wp) EPSABS, EPSREL, the absolute and relative accuracy requested.
    !
    !    Input, Integer(i4b) LIMIT, the upper bound on the number of subintervals that
...
70  continue 
  ...
  subroutine qawf ( f, a, omega, integr, epsabs, result, abserr, neval, ier )
    ...
    !    which evaluates the integrand function.
    !
    !    Input, Real(wp) A, the lower limit of integration.
    !
    !    Input, Real(wp) OMEGA, the parameter in the weight function.
    !
    !    Input, Integer(i4b) INTEGR, indicates which weight functions is used
    !    = 1, w(x) = cos(omega*x)
    !    = 2, w(x) = sin(omega*x)
    !
    !    Input, Real(wp) EPSABS, the absolute accuracy requested.
    !
    !    Output, Real(wp) RESULT, the estimated value of the integral.
    !
    !    Output, Real(wp) ABSERR, an estimate of || I - RESULT ||.
    ...
    !  Local parameters:
    !
    !    Integer(i4b) LIMLST, gives an upper bound on the number of cycles, LIMLST >= 3.
    !    if limlst < 3, the routine will end with ier = 6.
    !
    !    Integer(i4b) MAXP1, an upper bound on the number of Chebyshev moments which 
    ...
    Integer(i4b), parameter :: limit = 500
    Integer(i4b), parameter :: limlst = 50
    Integer(i4b), parameter :: maxp1 = 21

    Real(wp) a
    Real(wp) abserr
    Real(wp) alist(limit)
    Real(wp) blist(limit)
    Real(wp) chebmo(maxp1,25)
    Real(wp) elist(limit)
    Real(wp) epsabs
    Real(wp) erlst(limlst)
    Real(wp), external :: f
    Integer(i4b) ier
    Integer(i4b) integr
    Integer(i4b) iord(limit)
    Integer(i4b) ierlst(limlst)
    Integer(i4b) lst
    Integer(i4b) neval
    Integer(i4b) nnlog(limit)
    Real(wp) omega
    Real(wp) result
    Real(wp) rlist(limit)
    Real(wp) rslst(limlst)
  ...
  subroutine qawfe ( f, a, omega, integr, epsabs, limlst, limit, maxp1, &
    ...
    !    Input, Real(wp) OMEGA, the parameter in the weight function.
    !
    !    Input, Integer(i4b) INTEGR, indicates which weight function is used
    !    = 1      w(x) = cos(omega*x)
    !    = 2      w(x) = sin(omega*x)
    !
    !    Input, Real(wp) EPSABS, the absolute accuracy requested.
    !
    !    Input, Integer(i4b) LIMLST, an upper bound on the number of cycles.
    !    LIMLST must be at least 1.  In fact, if LIMLST < 3, the routine 
    !    will end with IER= 6.
    !
    !    Input, Integer(i4b) LIMIT, an upper bound on the number of subintervals 
    ...
    Integer(i4b) numrl2
    Real(wp) omega
    Real(wp), parameter :: p = 0.9E+00
    Real(wp), parameter :: pi = 3.1415926535897932E+00
    Real(wp) p1
    Real(wp) psum(52)
    Real(wp) reseps
    Real(wp) result
    ...
    if ( omega == 0.0e+00 ) then
       ...
       if ( integr == 1 ) then
          call qagi ( f, zero_wp, 1, epsabs, zero_wp, result, abserr, neval, ier )
       else
          result = 0.0E+00
          abserr = 0.0E+00
          neval = 0
    ...
    do lst = 1, limlst
       ...
       call qfour ( f, c1, c2, omega, integr, epsa, zero_wp, limit, lst, maxp1, &
...
80  continue
  ...
  subroutine qawo ( f, a, b, omega, integr, epsabs, epsrel, result, abserr, &
    ...
    !    Input, Real(wp) OMEGA, the parameter in the weight function.
    !
    !    Input, Integer(i4b) INTEGR, specifies the weight function:
    !    1, W(X) = cos ( OMEGA * X )
    !    2, W(X) = sin ( OMEGA * X )
    !
    !    Input, Real(wp) EPSABS, EPSREL, the absolute and relative accuracy requested.
  ...
  subroutine qaws ( f, a, b, alfa, beta, integr, epsabs, epsrel, result, &
    ...
    !    Input, Real(wp) A, B, the limits of integration.
    !
    !    Input, Real(wp) ALFA, BETA, parameters used in the weight function.
    !    ALFA and BETA should be greater than -1.
    !
    !    Input, Integer(i4b) INTEGR, indicates which weight function is to be used
    !    = 1  (x-a)**alfa*(b-x)**beta
  ...
  subroutine qawse ( f, a, b, alfa, beta, integr, epsabs, epsrel, limit, &
    ...
    !
    !    Output, Integer(i4b) LAST, the number of subintervals actually produced in 
    !    the subdivision process.
    !
    !  Local parameters:
    !
  ...
  subroutine qc25c ( f, a, b, c, result, abserr, krul, neval )
    ...
    !    Input, Real(wp) A, B, the limits of integration.
    !
    !    Input, Real(wp) C, the parameter in the weight function.
    !
    !    Output, Real(wp) RESULT, the estimated value of the integral.
    ...
    Real(wp) cheb12(13)
    Real(wp) cheb24(25)
    !Real(wp), external :: qwgtc    ! ---- IS commented this line on 10 Jul 2010
    Real(wp), external :: f
    Real(wp) fval(25)
    Real(wp) hlgth
    Integer(i4b) i
    ...
    Real(wp) res24
    Real(wp) u
    Real(wp), parameter, dimension ( 11 ) :: x = (/ &
  ...
  subroutine qc25o ( f, a, b, omega, integr, nrmom, maxp1, ksave, result, &
    ...
    !    = 2, w(x) = sin(omega*x)
    !
    !    ?, Integer(i4b) NRMOM, the length of interval (a,b) is equal to the length
    ...
    !    Output, Integer(i4b) NEVAL, the number of times the integral was evaluated.
    !
    !    Output, Real(wp) RESABS, approximation to the integral J.
    !
    !    Output, Real(wp) RESASC, approximation to the integral of abs(F-I/(B-A)).
    !
    !         on entry and return
    !           momcom - Integer(i4b)
    ...
    Real(wp) p3
    Real(wp) p4
    !Real(wp), external :: qwgto ! ---- IS commented this line on 10 Jul 2010
    Real(wp) resabs
    Real(wp) resasc
    Real(wp) resc12
    Real(wp) resc24
...
140 continue
  ...
  subroutine qc25s ( f, a, b, bl, br, alfa, beta, ri, rj, rg, rh, result, &
    ...
    !    Input, Real(wp) A, B, the limits of integration.
    !
    !    Input, Real(wp) BL, BR, the lower and upper limits of integration.
    !    A <= BL < BR <= B.
    !
    !    Input, Real(wp) ALFA, BETA, parameters in the weight function.
    !
    !    Input, Real(wp) RI(25), RJ(25), RG(25), RH(25), modified Chebyshev moments 
    !    for the application of the generalized Clenshaw-Curtis method,
    !    computed in QMOMO.
    !
    !    Output, Real(wp) RESULT, the estimated value of the integral, computed by 
    ...
    !    Output, Real(wp) ABSERR, an estimate of || I - RESULT ||.
    !
    !    Output, Real(wp) RESASC, approximation to the integral of abs(F*W-I/(B-A)).
    !
    !    Input, Integer(i4b) INTEGR,  determines the weight function
    ...
    Integer(i4b) isym
    Integer(i4b) neval
    !Real(wp), external :: qwgts ! ---- IS commented this line on 10 Jul 2010
    Real(wp) resabs
    Real(wp) resasc
    Real(wp) result
    Real(wp) res12
...
270 continue
  ...
  subroutine qcheb ( x, fval, cheb12, cheb24 )
    ...
    !  Parameters:
    !
    !    Input, Real(wp) X(11), contains the values of COS(K*PI/24), for K = 1 to 11.
    !
    !    Input/output, Real(wp) FVAL(25), the function values at the points
    ...
    !
    !    Output, Real(wp) CHEB12(13), the Chebyshev coefficients for degree 12.
    !
    !    Output, Real(wp) CHEB24(25), the Chebyshev coefficients for degree 24.
    !
    Use SFL_Precision, only : i4b
    implicit none
  ...
  subroutine qextr ( n, epstab, result, abserr, res3la, nres )
    ...
    !  Parameters:
    !
    !    Input, Integer(i4b) N, indicates the entry of EPSTAB which contains
    !    the new element in the first column of the epsilon table.
    !
    !    Input/output, Real(wp) EPSTAB(52), the two lower diagonals of the triangular
    !    epsilon table.  The elements are numbered starting at the right-hand 
    !    corner of the triangle.
    !
    !    Output, Real(wp) RESULT, the estimated value of the integral.
    !
    !    Output, Real(wp) ABSERR, estimate of the absolute error computed from
    !    RESULT and the 3 previous results.
    !
    !    ?, Real(wp) RES3LA(3), the last 3 results.
    !
    !    Input/output, Integer(i4b) NRES, the number of calls to the routine.  This
...
30     continue
  ...
  subroutine qfour ( f, a, b, omega, integr, epsabs, epsrel, limit, icall, &
    ...
    !    Input, Real(wp) A, B, the limits of integration.
    !
    !    Input, Real(wp) OMEGA, the multiplier of X in the weight function.
    !
    !    Input, Integer(i4b) INTEGR, indicates the weight functions to be used.
    !    = 1, w(x) = cos(omega*x)
    !    = 2, w(x) = sin(omega*x)
    !
    !    Input, Real(wp) EPSABS, EPSREL, the absolute and relative accuracy requested.
    !
    !    Input, Integer(i4b) LIMIT, the maximum number of subintervals of [A,B]
...
200 continue
  ...
  subroutine qk15 ( f, a, b, result, abserr, resabs, resasc )
    ...
    !    Output, Real(wp) ABSERR, an estimate of | I - RESULT |.
    !
    !    Output, Real(wp) RESABS, approximation to the integral of the absolute
    !    value of F.
    !
    !    Output, Real(wp) RESASC, approximation to the integral | F-I/(B-A) | 
    !    over [A,B].
    !
    !  Local Parameters:
    !
  ...
  subroutine qk15i ( f, boun, inf, a, b, result, abserr, resabs, resasc )
    ...
    !    which evaluates the integrand function.
    !
    !    Input, Real(wp) BOUN, the finite bound of the original integration range,
    !    or zero if INF is 2.
    !
    !    Input, Integer(i4b) INF, indicates the type of the interval.
    ...
    !
    !    Input, Real(wp) A, B, the limits of integration, over a subrange of [0,1].
    !
    !    Output, Real(wp) RESULT, the estimated value of the integral.
    ...
    !    value of F.
    !
    !    Output, Real(wp) RESASC, approximation to the integral of the
  ...
  subroutine qk15w ( f, w, p1, p2, p3, p4, kp, a, b, result, abserr, resabs, &
    ...
    !                       calling program.
    !
    !    ?, Real(wp) P1, P2, P3, P4, parameters in the weight function
    !
    !    Input, Integer(i4b) KP, key for indicating the type of weight function
    !
    !    Input, Real(wp) A, B, the limits of integration.
    !
    !    Output, Real(wp) RESULT, the estimated value of the integral.
  ...
  subroutine qmomo ( alfa, beta, ri, rj, rg, rh, integr )
    ...
    !  Parameters:
    !
    !    Input, Real(wp) ALFA, a parameter in the weight function w(x), ALFA > -1.
    !
    !    Input, Real(wp) BETA, a parameter in the weight function w(x), BETA > -1.
    !
    !           ri     - Real(wp)
...
70  continue
  ...
  subroutine qsort ( limit, last, maxerr, ermax, elist, iord, nrmax )
    ...
    !  Parameters:
    !
    !    Input, Integer(i4b) LIMIT, the maximum number of error estimates the list can
    !    contain.
    !
    !    Input, Integer(i4b) LAST, the current number of error estimates.
    !
    !    Input/output, Integer(i4b) MAXERR, the index in the list of the NRMAX-th 
    !    largest error.
    !
    !    Output, Real(wp) ERMAX, the NRMAX-th largest error = ELIST(MAXERR).
    !
    !    Input, Real(wp) ELIST(LIMIT), contains the error estimates.
    !
    !    Input/output, Integer(i4b) IORD(LAST).  The first K elements contain 
    ...
    !      K = LIMIT+1-LAST.
    !
    !    Input/output, Integer(i4b) NRMAX.
    !
    Use SFL_Precision, only : i4b
    implicit none
...
90  continue
  ...
  function qwgtc ( x, c, p2, p3, p4, kp )
    ...
    !  Parameters:
    !
    !    Input, Real(wp) X, the point at which the weight function is evaluated.
    !
    !    Input, Real(wp) C, the location of the singularity.
    !
    !    Input, Real(wp) P2, P3, P4, parameters that are not used.
    !
    !    Input, Integer(i4b) KP, a parameter that is not used.
    !
    !    Output, Real(wp) QWGTC, the value of the weight function at X.
    !
    Use SFL_Precision, only : i4b
    implicit none
  ...
  function qwgto( x, omega, p2, p3, p4, integr )
    ...
    !    Input, Real(wp) X, the point at which the weight function is evaluated.
    !
    !    Input, Real(wp) OMEGA, the factor multiplying X.
    !
    !    Input, Real(wp) P2, P3, P4, parameters that are not used.
    !
    !    Input, Integer(i4b) INTEGR, specifies which weight function is used:
    !    1. W(X) = cos ( OMEGA * X )
    !    2, W(X) = sin ( OMEGA * X )
    !
    !    Output, Real(wp) QWGTO, the value of the weight function at X.
    !
    Use SFL_Precision, only : i4b
    implicit none
  ...
  function qwgts ( x, a, b, alfa, beta, integr )
    ...
    !    Input, Real(wp) X, the point at which the weight function is evaluated.
    !
    !    Input, Real(wp) A, B, the endpoints of the integration interval.
    !
    !    Input, Real(wp) ALFA, BETA, exponents that occur in the weight function.
    !
    !    Input, Integer(i4b) INTEGR, specifies which weight function is used:
    !    1. W(X) = (X-A)**ALFA * (B-X)**BETA
    !    2, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A)
    ...
    !    4, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A) * log(B-X)
    !
    !    Output, Real(wp) QWGTS, the value of the weight function at X.
    !
    Use SFL_Precision, only : i4b
    implicit none
